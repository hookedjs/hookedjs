...
export default function Portal(p: PortalProps) {
  const {placement = 'center', message, duration, styled = true, onDismiss, dismissable = true} = p,
  ...
  return (
    <PortalOuter data-placement={placement} data-styled={styled}>
      <div>
        <div ref={ref as any}>{message}</div>
      </div>
    </PortalOuter>
  )
  ...
  function init() {
    setDismissed(false)
    if (duration) setTimeout(() => setTimedOut(true), duration)
  }
}
...
Portal.prompt = async <ResponseType extends any>(
  ...
  PortalStore.setValue({
    message: PromptC({resolve: _resolve}),
    duration,
    placement,
    dismissable,
    onDismiss,
...
Portal.confirm = ({message = 'Are you sure?', body = ''} = {}) => {
  return Portal.prompt<boolean>(({resolve}) => (
    <ConfirmPrompt resolve={resolve}>{body || <p>{message}</p>}</ConfirmPrompt>
  ))
}
...
function ConfirmPrompt({
  ...
  return (
    <div>
      <div>{children}</div>
      <ButtonGroup>
        <button onClick={() => resolve(true)} class="primary large">
          Proceed
        </button>
        <button onClick={() => resolve(false)} class="link">
          Cancel
        </button>
      </ButtonGroup>
    </div>
