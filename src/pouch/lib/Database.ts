...
class Database {
  ...
  async sync() {
    if (this.host) {
      await this._db.sync(this._remote, {retry: true}).catch(err => {
  ...
  async set<T extends IStandardFieldsCreate>(doc: T): Promise<T & IStandardFields> {
    ...
    const res = await this._db.put(doc2, {}).then(idAndRev => ({...doc2, _rev: idAndRev.rev}))
    return res
  }
  async setMany<T extends IStandardFieldsCreate>(docs: T[]) {
    ...
    const res = await this._db.bulkDocs<any>(docs2, {}).then(idAndRevs => {
  ...
  async find<T extends IStandardFields>(props: IFindProps<T>): Promise<T[]> {
    ...
    if (
      (typeof props.selector?._id === 'string' || props.selector?._id?.$in) &&
      ...
      if (typeof props.selector?._id === 'string')
        cached.fetchP = this.get<T>(props.selector._id).then(doc => {
          if (doc.deletedAt) throw new NotFoundError(props.selector!._id as string)
          return {docs: [doc]}
        })
      else
        cached.fetchP = this.get<T>(props.selector._id.$in as string[]).then(docs => ({
    ...
    // Else do full search
      ...
      if (!propsMapped.selector.deletedAt) propsMapped.selector.deletedAt = {$exists: false}
      cached.fetchP = this._db.find(propsMapped).then(res => {
  ...
  async delete<T extends IStandardFields>(doc: T): Promise<T> {
    ...
    const res = this._db.put<T>(doc2, {}).then(idAndRev => ({...doc2, _rev: idAndRev.rev}))
    return res
  }
