  ...
  async validate() {
    return assertValidSet<IStandardFields & ITenantPersonExtra>(this.values, {
      ...
      type: assertValid('type', this.type, [], {
        isEqual: {
          expected: TenantPerson.type,
          message: `type must be ${TenantPerson.type}`,
        },
      }),
      ...
      status: assertValid('status', this.status, ['isRequired', 'isString'], {
        isOneOfSet: TenantPersonStatusSet,
      }),
      roles: assertValid('roles', this.roles, ['isRequired', 'isArray', 'isNoneEmpty'], {
        arrayValuesAreOneOfSet: TenantPersonRoleSet,
      }),
